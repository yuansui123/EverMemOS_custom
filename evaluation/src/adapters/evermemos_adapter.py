...
class EverMemOSAdapter(BaseAdapter):
    ...
    async def add(
        ...
        **kwargs
        ...
        if len(pending_conversations) == 0:
            console.print(f"\n🎉 All conversations completed, skipping MemCell extraction!", style="bold green")
        else:
            ...
            with Progress(
                ...
                for conv in pending_conversations:
                    conv_id = conv.conversation_id
                    conv_index = self._extract_conv_index(conv_id)  # Extract numeric index
                    conv_task_id = progress.add_task(
            ...
            console.print(f"   ✅ Successfully processed: {successful_convs}/{len(pending_conversations)}", style="green")
            console.print(f"   📝 Total memcells: {total_memcells}", style="blue")
            console.print(f"   ⏱️  Total time: {elapsed:.2f}s", style="yellow")
            if len(pending_conversations) > 0:
                console.print(f"   🚀 Average per conversation: {elapsed/len(pending_conversations):.2f}s", style="cyan")
        ...
        console.print(f"   BM25 index: need to build {bm25_to_build}, existing {total_convs - bm25_to_build}")
        if use_hybrid:
            console.print(f"   Embedding index: need to build {emb_to_build}, existing {total_convs - emb_to_build}")
        
        # Build BM25 index
        if bm25_to_build > 0:
            console.print(f"\n🔨 Building BM25 index ({bm25_to_build} conversations)...", style="yellow")
        ...
        if use_hybrid:
            if emb_to_build > 0:
                console.print(f"\n🔨 Building Embedding index ({emb_to_build} conversations)...", style="yellow")
                ...
                console.print("✅ All Embedding indexes exist, skipping build", style="green")
        ...
        console.print(f"   💡 Using lazy loading strategy (memory-friendly)", style="cyan")
        console.print(f"{'='*60}\n", style="dim")
    ...
    async def search(self, query: str, conversation_id: str, index: Any, **kwargs) -> SearchResult:
        ...
        if conversation and top_results:
            ...
            # Build context
            retrieved_docs_text = []
            for doc, score in top_results[:response_top_k]:  # Use response_top_k from config
